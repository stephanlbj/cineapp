...
export const MovieService = {
  ...
  async searchMovies(query: string): Promise<Movie[]> {
    ...
    try {
      const { apiBaseUrl, apiKey } = getApiConfig()
      const response = await $fetch<{ results: Movie[] }>(`${apiBaseUrl}/search/movie`, {
        method: 'GET',
        params: {
          query,
          api_key: apiKey,
  ...
  async getMovieDetails(id: number): Promise<Movie | null> {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(`Erreur lors de la récupération des détails du film: ${error.message}`)
      } else {
        throw new Error('Erreur inconnue lors de la récupération des détails du film.')
      }
    }
  },
}
